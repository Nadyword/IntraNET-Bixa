...
public class PrestacionesDocument(TramiteReportModel model) : IDocument
{
    private const string BorderColor = "#333333";
    private const string GrayBg = "#EEEEEE";
    private const string LabelColor = "#555555";

    private const string Revision = "00";
    private bool EsPrestamo => model.Prestaciones?.EsPrestamo ?? false;
    private string Titulo => EsPrestamo ? "SOLICITUD DE PRESTAMO PRESTACIONES SOCIALES" : "SOLICITUD DE PRESTACIONES SOCIALES";
    private string Codigo => EsPrestamo ? "5-SPP-RH-001" : "5-SPS-RH-001";
        ...
        container.Border(2).BorderColor(BorderColor).Row(row =>
        {
            ...
            row.RelativeItem(1).Padding(10).Column(col =>
            {
                col.Item().BorderBottom(1).BorderColor(GrayBg).PaddingBottom(4)
                   .Text(t => { t.Span("Código: ").Bold().FontSize(8); t.Span(Codigo).FontSize(8); });
                col.Item().PaddingTop(4).BorderBottom(1).BorderColor(GrayBg).PaddingBottom(4)
                   .Text(t => { t.Span("Revisión: ").Bold().FontSize(8); t.Span(Revision).FontSize(8); });
    ...
    private void ComposeDatosSolicitante(IContainer container)
    {
        container.Column(col =>
        {
            ...
            col.Item().Row(row =>
            {
                ...
                row.RelativeItem(1).Element(c => FormField(c, "C.I.", model.EmpleadoCi));
            });
    ...
    private void ComposeDetalleSolicitud(IContainer container)
    {
        container.Border(1).BorderColor(BorderColor).Padding(15).Column(col =>
        {
            ...
            {
                t.Span(EsPrestamo
                    ? "Solicito me sea concedido un préstamo en base a mis prestaciones de antigüedad, por Bs. "
                    : "Solicito me sea concedido un anticipo en base a mis prestaciones de antigüedad, por Bs. ").Bold();
                t.Span(model.Prestaciones?.Monto.ToString("N2") ?? "—");
            });

            col.Item().Text(t =>
            {
                t.Span("CANTIDAD DESTINADA A: ").Bold();
                t.Span(model.Prestaciones?.Destino ?? "—");
            });

            if (EsPrestamo && model.Prestaciones?.Cuotas is > 0)
            {
                col.Item().Text(t =>
                {
                    t.Span("CUOTAS: ").Bold();
                    t.Span($"{model.Prestaciones.Cuotas} de Bs. {model.Prestaciones.MontoCuota:N2}");
                });
            }

            if (!string.IsNullOrWhiteSpace(model.Prestaciones?.Observaciones))
            {
                col.Item().Text(t =>
                {
                    t.Span("OBSERVACIONES: ").Bold();
                    t.Span(model.Prestaciones!.Observaciones!);
                });
            }
        });
    }
