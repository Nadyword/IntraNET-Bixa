...
4. **Configura la base de datos y aplica las migraciones:**
   ...
   - **Aplica las migraciones existentes:**
     ```bash
     dotnet ef database update -p Bixa.Backend.DataAccess -s Bixa.Backend --context AppDbContext
     ```
   ...
   - **Comandos de Entity Framework:**
     ...
     - Para crear una nueva migración:
       ```bash
       dotnet ef migrations add MigrationV38P --context AppDbContext -p Bixa.Backend.DataAccess -s Bixa.Backend
       ```
   ...
   ```
        ...
        cd /var/www/IntraNET-Bixa/Publicacion/
        chmod +x ./Bixa.Backend
    ```
